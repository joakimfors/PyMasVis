...
MasVis can generate two types of analyses, one overview graph for several tracks and one detailed analysis for a single track.

## The Overview graph

The overview graph is appropriate for displaying condensed information about an entire album. It shows the waveforms from the left and right channels superimposed on each other. The left channel is shown in blue, the right in red. Parts where left and right channels overlap are shown in black. If a lot of color is shown, the channels are quite different.
The crest factor and the level of the highest peak are shown to the right of each of the graphs. The values can differ from those in the detailed analysis, since the left and right channels are analyzed as one.
